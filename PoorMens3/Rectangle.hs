...
rectangle x y w h = shape x y $ Rectanglish { getWidth = w
                                            , getHeight = h
                                            , rectangleExt = ()
                                            }
...
setHeight i s = s { shapeExt = (shapeExt s) { getHeight = i } }

setWidth :: Int -> Rectangle w -> Rectangle w
setWidth i s = s { shapeExt = (shapeExt s) { getWidth = i } }
...
instance Draw (Rectanglish w)
 where
  draw s =  putStrLn ("Drawing a Rectangle at:("
         ...
         ++ ","
         ++ (show (getY s))
         ++ "), width " ++ (show (getWidth (shapeExt s)))
         ++ ", height " ++ (show (getHeight (shapeExt s))))
